  ...
  for (const workspace of workspaces) {
    ...
    if (!packages || packages.length === 0) {
      errors.push({
        ...
        message: `Couldn't find any packages in the workspace.`,
      });
      return errors;
    }
    ...
    for (const pkg of packages) {
      ...
      if (devDependencies) {
        ...
        if (badDeps.length > 0) {
          errors.push({
            type: 'Error',
            packageName: pathToPkg,
            message: `Package "${pkg}" cannot have devDependencies: "${badDeps}" in its package.json. This devDependency should be placed in the root package.json.`,
          });
        }
      }
    }
